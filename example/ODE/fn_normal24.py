...
for ii in range(1,13):
    ...
    writer = csv.writer(f)
    writer.writerow(['key','S', 'Ic', 'dI/dt','R','Z', 'E','I'])
    ...
    writer.writerow ( [0,NN,1,0,0,0,0,1 ] )
    for x1 in range ( 1, 301 ):
    ...
    for x1 in range ( 1, 301 ):
        ...
        z = N2[x1-1]-N[x1-1]
        Z.append ( z )  # Z怀疑者
        writer.writerow ( [key,s,Nc[x1-1],vi[x1-1],R[x1-1],z,e,N[x1-1] ] )
        key=key+1
    ...
    plt.sca(ax1)
    plt.plot(t, Nc[0:300:1], color='blue',label='0.5h',marker = "x",markersize=4)  #所有值，间隔半个小时
    plt.sca(ax2)
    plt.plot(t, Sc[0:300:1], color='red',label='0.5h',marker = "x",markersize=4)
