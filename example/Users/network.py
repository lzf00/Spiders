...
#广度优先遍历，生成传播网络
    ...
    out = open ( 'Network.csv', 'a', newline='' )         #‘a’追加内容， 使用newline（）使文件不空行
    csv_write = csv.writer ( out, dialect='excel' )  #以excel格式写文件
    #csv_write.writerow ( stu3 )
    stu = [netidnum, count, rls, h, 'fake',distance]
    csv_write.writerow ( stu )
