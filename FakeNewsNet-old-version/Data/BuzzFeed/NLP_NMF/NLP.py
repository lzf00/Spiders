...
for x in d['text']:
        try:
            ...
            print ( '1111111111' )
            cn = cut_model.tokenize ( x )     #nltk进行英文分词
...
#d = pd.read_csv('X疫情期间假新闻1.20-4.8.csv', usecols=['内容'])#pandas读文件某一列
d1 = pd.read_csv('BuzzFeed_Real_1-Webpage.csv', usecols=['text'] )#pandas读文件某一列
#print(d)
#file_path='NLP_plus.csv'
#file_path = 'F_NLP.csv'
file_path1 = 'NLP_Real.csv'
...
for x in d1['text']:
        try:
            ...
            cut_model = nltk.WordPunctTokenizer()
            print('1111111111')
            cn1 = cut_model.tokenize ( x )
            #cn1 = nltk.word_tokenize(x)  # nltk进行英文分词
