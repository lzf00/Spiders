...
def train(V, r, k, e):
    ...
    H = sp.rand ( r, n, density=1, dtype=float ).todense ()  # 生成一个随机稀疏矩阵r*n，密度为0~1
...
if __name__ == "__main__":
    ...
    W, H ,error= train(V, 10, 1000, 1e-9) #原始矩阵V，特征数10，迭代次数50，误差1e-5=0.00001
    #print (V)
    print('W------------------------------------------------------------------------')
    print (W)
    print('H------------------------------------------------------------------------')
    ...
    print('W*H----------------------------------------------------------------------')
    print (W * H)
    np.savetxt ( 'Fake_mat_W.csv', W, delimiter=' ' ) #保存矩阵W
    np.savetxt ( 'Fake_mat_H.csv', H, delimiter=' ' ) #保存矩阵H
