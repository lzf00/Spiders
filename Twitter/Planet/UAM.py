...
import pyautogui
key = 'UAM'
file_path = 'D:\All_Script\Python_planet_picture\\test_UAM1.18/' + key + '.csv'
...
name = '157909194@qq.com' #12.22创建，14天，1月3号爬取
...
time.sleep ( 2 )
driver.find_element_by_xpath ( '//input[@class="MuiInputBase-input"]' ).send_keys ( 'Andersen Airforce Base, Yigo, Guam' )
time.sleep ( 5 )
# 测试
...
i = 1
for date in all_date[50:]:
    try:
            i = i + 1
        # if (i > 40):
            time.sleep(1)
            date.find_element_by_xpath ('./div[@class="MuiAvatar-root MuiAvatar-circle jss996 MuiAvatar-colorDefault"]' ).click ()#图片下的日期按钮
            time.sleep ( 5 )
            print ( "11111" )#测试代码
            data = driver.find_element_by_xpath ('//div[@class="MuiButtonBase-root MuiListItem-root jss1447 MuiListItem-gutters MuiListItem-divider MuiListItem-button MuiListItem-secondaryAction Mui-selected jss1448"]/div[2]/div[1]/h6' )
            # 显示图片时间
            print ( "22222" )
            ...
            print ( text1 )
            pyautogui.screenshot( 'D:\All_Script\Python_planet_picture\\test_UAM1.18/' + key + '_' + text1 + '.png' ,region=(850, 340, 476, 476))
            ...
            time.sleep(1)
            pyautogui.click(x=1380, y=460, duration=2)
            pyautogui.scroll ( 930, x=1380, y=460 )
            time.sleep ( 3 )
            pyautogui.screenshot( 'D:\All_Script\Python_planet_picture\\test_UAM1.18/' + key + '_' + text1 +'_T'+ '.png' ,region=(485, 152, 1256, 890))
            #driver.get_screenshot_as_file ('D:\All_Script\Python_planet_picture\\test/' + key + '_' + text1 +'_T'+ '.png' )  # 截图并以日期命名T
            print ( '已截图222' )
            pyautogui.scroll ( -930, x=1380, y=460 )
            time.sleep(1)
