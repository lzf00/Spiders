...
import pyautogui
key = 'DNA'
file_path = 'D:\All_Script\Python_planet_picture\\DNA/' + key + '1.csv'
...
time.sleep ( 2 )
driver.find_element_by_xpath ( '//input[@class="MuiInputBase-input"]' ).send_keys ( ''' 26° 21' 20.16" N,127° 46' 4.79" E ''' )
time.sleep ( 5 )
# 测试
...
for date in all_date:
    try:
        time.sleep(1)
        date.find_element_by_xpath ('./div[@class="MuiAvatar-root MuiAvatar-circle jss1162 MuiAvatar-colorDefault"]' ).click ()#图片下的日期按钮
        time.sleep ( 8 )
        print ( "11111" )#测试代码
        data = driver.find_element_by_xpath ('//div[@class="MuiButtonBase-root MuiListItem-root jss1706 MuiListItem-gutters MuiListItem-divider MuiListItem-button MuiListItem-secondaryAction Mui-selected jss1707"]/div[2]/div[1]/h6' )
        # 显示图片时间
        print ( "22222" )
        ...
        print ( text1 )
        time.sleep(2)
        pyautogui.screenshot( 'D:\All_Script\Python_planet_picture\\DNA/' + key + '_' + text1 + '.png' ,region=(1126, 171, 646, 646))#顶点位置，图片尺寸
        ...
        time.sleep(1)
        pyautogui.scroll (  280, x, y, _pause=1 )

        time.sleep ( 8 )
        #pyautogui.screenshot( 'D:\All_Script\Python_planet_picture\\UTP/' + key + '_' + text1 +'_T'+ '.png' ,region=(720, 325, 1021, 690))
        pyautogui.screenshot( 'D:\All_Script\Python_planet_picture\\DNA/' + key + '_' + text1 +'_T'+ '.png' ,region=(485, 152, 1256, 890))
        #driver.find_element_by_xpath ('//button[@class="MuiButtonBase-root MuiFab-root MuiSpeedDialAction-fab jss334 jss342 MuiFab-sizeSmall"]' ).click ()#点击放大按钮
        print ( '已截图222' )
        pyautogui.scroll ( -280, x, y, _pause=1 )
        time.sleep(1)
