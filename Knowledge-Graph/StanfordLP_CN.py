...
import json
nlp = StanfordCoreNLP(r'D:\Anaconda3\stanford-corenlp-4.3.1',lang='zh')
#分词（word_tokenize）、词性标注（pos_tag）、命名实体识别（ner）、句法依存分析（dependency_parse）、句法解析（parse）
file_path='D:\All_Script\Python_deagel/Stanford_CN.csv'
...
d1 = pd.read_csv ( filepath1, usecols=['chinese_content'] )
print(d1[0:])
d = pd.read_csv ( 'news.csv', usecols=['chinese_content'] )

key=0
sentence = 'Guangdong University of Foreign Studies is located in Guangzhou.'
#print ('Tokenize:', nlp.word_tokenize(sentence))
s1="StanfordCoreNLP是一个集成的框架。它的目标是使得应用一大堆语言分析工具分析大量的文本变得简单。"
print ('Named Entities:', nlp.ner(s1))
for x in d1['chinese_content'][0:10]:  #424对应CSV426
    #try:
